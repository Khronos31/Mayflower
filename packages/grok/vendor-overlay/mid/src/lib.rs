...
mod ios {
    ...
    pub(crate) fn get_mid_result() -> Result<String, MIDError> {
        let machine =
            run_shell_command("/usr/sbin/sysctl", ["-n", "hw.machine"]).unwrap_or_default();
        let uuid = run_shell_command("/usr/sbin/sysctl", ["-n", "kern.uuid"]).unwrap_or_default();
        let combined = format!("{}|{}", machine.trim(), uuid.trim());
        if combined.trim_matches('|').is_empty() {
